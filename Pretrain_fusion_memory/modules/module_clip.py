...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,  video_frame=-1, flag=None):
        ...
        #上述操作，得到一个（2*12，1+49， 512)的tensor，前12行为第一个样本的12帧，后12行为第二个样本的12帧，每帧里面50个token，每个token 512维
        x = self.ln_pre(x)
        #pytorch自带的multiheadattention必须先转变tensor，转变完后的tensor为（50， 24， 512)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x, video_frame=video_frame, task='vision')
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.fusion_positional_embedding = nn.Parameter(torch.empty(self.context_length, transformer_width))
        #self.fusion_vtm_linear = nn.Linear(transformer_width, 2)
        #self.fusion_vtm_linear = nn.Sequential(OrderedDict([
        #   ("c_fc", nn.Linear(1024,1024)),
        #   ("c_proj", nn.Linear(1024,2))
        #]))
        self.fusion_ln = LayerNorm(1024)
        self.fusion_query = nn.Parameter(512 ** -0.5 * torch.randn(8, 1024, 512))
        self.fusion_memory = nn.Parameter(512 ** -0.5 * torch.randn(8192, 512))
    ...
    @staticmethod
    def get_config(pretrained_clip_name="ViT-B/16"):
        model_path = os.path.join("/apdcephfs/share_1324356/zifengchai/smart/data/data_CLIP4clip/modules", "ViT-B-16.pt")
    ...
    def encode_image(self, image, return_hidden=False, video_frame=-1, flag=None):
        hidden = self.visual(image.type(self.dtype),video_frame=video_frame, flag=flag)
    ...
    def encode_text(self, text,mask, return_hidden=False):
